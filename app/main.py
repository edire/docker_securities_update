...
df_send = con.read(f'select * from {sql_table} where `Date` >= DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL -30 DAY)')
...
SendEmail(
    ...
    , subject=os.getenv('email_subject') + ' - ' + dt.date.today().strftime('%m-%d-%Y')
    , body=body
